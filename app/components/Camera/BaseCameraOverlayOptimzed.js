...
import OptimizedOverlay from "../OptimizedUI/OptimizedOverlay";
import FlashController from "./FlashController";
import FlashGradient from "./FlashGradient";

const { width: screenWidth, height: screenHeight } = Dimensions.get("window");

function BaseCameraOverlayOptimized({ onPhotoTaken }) {
  const cameraRef = useRef(null);
  const [capturedPhoto, setCapturedPhoto] = useState(null);
  const [complianceCheckPhoto, setComplianceCheckPhoto] = useState(null);
  const [facing, setFacing] = useState("front");
  const [flashMode, setFlashMode] = useState(false);
  const [flashBrightness, setFlashBrightness] = useState(0.1);
  const passportRatio = 35 / 45;
  const overrideCaptureRef = useRef(false);
  ...
  async function capturePhoto() {
    ...
    setComplianceCheckPhoto(processed);
  }
  ...
  const optimizedOverlayRef = useRef(null);

  function handleCapturePress() {
    const overlay = optimizedOverlayRef.current;

    if (!overlay) return;

    const allStepsValid = overlay.areAllStepsValid();

    if (!allStepsValid && !overrideCaptureRef.current) {
      overrideCaptureRef.current = true;

      overlay.showWarningHint();

      setTimeout(() => {
        overrideCaptureRef.current = false;
      }, 3000);
    ...
    <View style={styles.container}>
      {capturedPhoto ? (
        ...
        <>
          ...
          />
          <OptimizedOverlay
            ref={optimizedOverlayRef}
            container={[
              styles.passportOverlay,
              {
                top: topOverlayHeight,
                height: visibleHeight,
              },
            ]}
            photo={complianceCheckPhoto}
            capturePhoto={capturePhoto}
          />
          <View style={styles.bottomCenter}>
            <CaptureButton
              onPress={handleCapturePress}
              ready={optimizedOverlayRef.current?.areAllStepsValid()}
              displayReadyAnimation={optimizedOverlayRef.current?.areAllStepsValid()}
            />
          </View>
...
export default BaseCameraOverlayOptimized;
