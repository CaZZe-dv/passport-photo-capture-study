...
import { SUS_QUESTIONS } from "../../constants/QuestionsData";
...
function SUSQuestionnaireScreen({ navigation }) {
  const studyResultsContext = useContext(StudyResultsContext);
  const questions = SUS_QUESTIONS;
  ...
  function onPressNextQuestionHandler() {
    studyResultsContext.updateStudyData((prev) => {
      return {
        ...prev,
        [`sus_question${currentQuestion.id}`]: convertAnswerToNumber(
    ...
    if (questionIndex < questions.length - 1) {
      ...
      navigation.replace("CustomLikertQuestionnaireScreen");
    }
  }
...
export default SUSQuestionnaireScreen;
